...
import org.hyperledger.besu.ethereum.stateless.bintrie.factory.ValueNodeFactory;
 ...
 */
public class StoredValueNode<K extends BitSequence<K>, V> extends LeafNode<K, V> {
  final ValueNodeFactory<K, V> nodeFactory;
  Optional<LeafNode<K, V>> loadedNode;
   ...
   */
  public StoredValueNode(final ValueNodeFactory<K, V> nodeFactory, final Optional<K> location) {
    super(location);
   ...
   */
  public StoredValueNode(
      final ValueNodeFactory<K, V> nodeFactory,
      final Optional<K> location,
      final Optional<Bytes32> commitment) {
    super(location, Optional.empty(), commitment);
    this.nodeFactory = nodeFactory;
    loadedNode = Optional.empty();
  }
   ...
   */
  @Override
  public StoredValueNode<K, V> setLocation(Optional<K> newLocation) {
    StoredValueNode<K, V> result = new StoredValueNode<K, V>(nodeFactory, newLocation);
   ...
   */
  @Override
  public StoredValueNode<K, V> setCommitment(Optional<Bytes32> newCommitment) {
    StoredValueNode<K, V> result = new StoredValueNode<K, V>(nodeFactory, location, newCommitment);
   ...
   */
  @Override
  public StoredValueNode<K, V> replaceLocation(K newLocation) {
    StoredValueNode<K, V> result = new StoredValueNode<K, V>(nodeFactory, Optional.of(newLocation));
  ...
  @Override
  public Bytes encode() {
    throw new RuntimeException("Should load StoredValueNode before getEncodedValue");
  }
  ...
  @Override
  public String print() {
    return String.format("StoredValue %s", location);
  }
  ...
  Optional<LeafNode<K, V>> retrieve(K location) {
